...
int isLess(int x, int y) {
   int signDiff = (((x >> 31) & 1) ^ ((y >> 31) & 1)) & ((x >> 31) & 1);//If the sign of x and y are different, save the sign of x in the variable signDiff
   int yNeg = ~y + 1; //compute the negative of y
   int signSame = (!(((x >> 31) & 1) ^ ((y >> 31) & 1))) & (((x + yNeg) >> 31) & 1); //If the sign of x and y are same, save the sign of x - y in the variable signSame
...
unsigned float_abs(unsigned uf) {
   ...
   //The code below checks whether the input uf is NaN by determining if the bits of e and f meet the conditions for NaN.
  if ((((uf >> 23) & 0x000000ff) == 0x000000ff) && (((uf << 9) | 0) != 0)) return uf; //if argument is Nan, return argument
...
unsigned float_twice(unsigned uf) {
   ...
   //if e = 000...0(denomarlized values), do left-shift by 1 bit to uf to implement 2*uf
...
unsigned float_i2f(int x) {
   ...
   if(exp < 23){
      ...
      f = shiftF >> 1;
      if(r && (s || (f&1))) f += 1; //(r && s) || (r && (f & 1)) change due to limit # of max operation
   }
